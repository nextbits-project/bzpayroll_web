...
import com.bzpayroll.dashboard.purchase.forms.PrintLabelDto;
...
//					.getShprovince(), c.getShcountry(), c.getShzipCode(), "0");
	...
	public boolean updateInsertVendor(String cvId, VendorDto c, String compID,
		...
		try {
			...
			updateVendorCreditCard(cvID, c.getCcType(), c.getCardNo(), c
					.getExpDate(), c.getCw2(), c.getCardHolderName(), c
					.getCardBillAddress(), c.getCardZip());
	...
	public ArrayList getPrintLabelInfo(HttpServletRequest request, String compId,int startValue,int limit) {
		...
		try {
			...
			while (rs_client.next()) {
				...
				vendor.setState(cs.getStatesName(rs_client
						.getString("State")));
