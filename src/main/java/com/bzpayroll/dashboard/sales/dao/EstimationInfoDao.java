...
public class EstimationInfoDao {
	...
	public ArrayList customerDetails(String compId, HttpServletRequest request) {
		...
		try {
			...
			while (rs.next()) {
				...
				objList.add(new LabelValueBean(rs.getString(3) + " , " + rs.getString(2), cvId));
	...
	public void getCountry(HttpServletRequest request, String country, UpdateInvoiceForm form) {
		Connection con = null;
		PreparedStatement pstmt1 = null;
	...
	public ArrayList getRecord(HttpServletRequest request, EstimationForm form, String compId, long estNo) {
		Connection con = null;
		PreparedStatement pstmt = null;
		...
		try {
			...
			if (rs.next()) {
				form.setBsAddressID(rs.getString("BSAddressID"));
				form.setBillTo(rs.getString(3) + rs.getString(4) + " " + rs.getString(5) + "\n" + rs.getString(6) + "\n"
	...
	public long getNextEstimationNo(String compId, String ordNo) {
		...
		try {
			String sql = "select EstNum from bca_invoice where EstNum > ?"
					+ " and CompanyID = ? and invoiceStatus in (0,2) and InvoiceTypeID=10" + " order by EstNum asc";
			pstmt = con.prepareStatement(sql);
	...
	public long getPreviousEstimationNo(String compId, String ordNo) {
		...
		try {
			String sql = "select EstNum from bca_invoice where EstNum < ?"
					+ " and CompanyID = ? and invoiceStatus in (0,2) and InvoiceTypeID=10" + " order by EstNum desc";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, ordNo);
			pstmt.setString(2, compId);
			rs = pstmt.executeQuery();
