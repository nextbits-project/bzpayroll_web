...
public class InvoiceInfoDao {
	...
	public void SalesUpdate(String compId, InvoiceDto form, int salesOrderType, int invoiceID) {
		...
		try {
			...
			if (shipped!=null && shipped.equals("on")) {
				pstmt1.setInt(23, 1);
	...
	public void Update(String compId, InvoiceDto form, int invoiceID, String custID) {
		...
		try {
			...
			String isPending = form.getIsPending()!=null?form.getIsPending():"off";
			pstmt1.setString(32,"1");
			pstmt1.setString(33,"1");
			pstmt1.setInt(34, isPending.equals("on")? 1:0);//pending
	...
	public void SearchCustomer(String compId, String cvId, HttpServletRequest request) {
		...
		try {
			...
			sqlString.append("and ( bca_clientvendor.Deleted = '0') and CompanyID='1' and bca_clientvendor.ClientVendorID ='" + cvId + "' group by ( bca_clientvendor.ClientVendorID )");
...
//				form.setIsPending(rs.getInt("InvoiceTypeID") == 7 ? "true": "false");
	...
	public Map<Long, Long> getInvoiceOrderNumbers(String compId, String orderBy) {
		...
		try {
			String sql = "select InvoiceID,OrderNum from bca_invoice where CompanyID =? "
				+ "and invoiceStatus in (0,2) and InvoiceTypeID = 1 and OrderNum !=0 order by OrderNum "+orderBy;
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, compId);
			rs = pstmt.executeQuery();
			while (rs.next()) {
				orderNumbers.put(rs.getLong(1), rs.getLong(2));
			}
		} catch (SQLException ex) {
	...
	public Map<Long, Long> getSalesOrderNumbers(String compId, String orderBy) {
		Map<Long, Long> orderNumbers = new LinkedHashMap<>();
		Connection con = null ;
		PreparedStatement pstmt = null;
		...
		try {
			String sql = "select InvoiceID, SONum from bca_invoice where CompanyID =? and invoiceStatus in (0,2 )"
					+ " and InvoiceTypeID in (7) order by SONum "+orderBy;
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, compId);
			rs = pstmt.executeQuery();
			if (rs.next()) {
