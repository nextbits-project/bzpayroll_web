/*
	...
	*	Description:
	*	Inserts Countries and/or bd_states as Dropdown List
	*	How to Use:
		...
		In Body Section:
		----------------
		Select bd_country (with bd_states):   <select id="bd_country" name ="bd_country"></select>
			
		Select bd_state: <select name ="bd_state" id ="bd_state"></select>

        Select bd_country (without bd_states):   <select id="bd_country2" name ="bd_country2"></select>
			
		<script language="javascript">
			populateCountries("bd_country", "bd_state");
			populateCountries("bd_country2");
		</script>
...
s_a[34]="Ayeyarwady|Bago|Chin bd_state|Kachin bd_state|Kayah bd_state|Kayin bd_state|Magway|Mandalay|Mon bd_state|Rakhine bd_state|Sagaing|Shan bd_state|Tanintharyi|Yangon";
...
s_a[209]="Eastern Cape|Free bd_state|Gauteng|KwaZulu-Natal|Mpumalanga|North-West|Northern Cape|Northern Province|Western Cape";
...
function populatebd_states( bd_countryElementId, bd_stateElementId, state ){
	var selectedbd_countryIndex = document.getElementById( bd_countryElementId ).selectedIndex;
	var bd_stateElement = document.getElementById( bd_stateElementId );
	bd_stateElement.length=0;
	if(selectedbd_countryIndex != 38){
		bd_stateElement.options[0] = new Option('Select State','');
	}
	else{bd_stateElement.options[0] = new Option('Select Province','');}	
	bd_stateElement.selectedIndex = 0;
	if(selectedbd_countryIndex == 1){
		var UsaArray = {'AL': 'Alabama', "AK": 'Alaska', 'AZ':'Arizona', 'AR':'Arkansas', 'CA':'California', 'CO':'Colorado', 'CT': 'Connecticut', 'DE':'Delaware', 'FL':'Florida', 'DC':'District of Columbia', 'FL':'Florida', 'GA':'Georgia', 'HI':'Hawaii', 'ID':'Idaho', 'IL':'Illinois', 'IN':'Indiana',
			'IA':'Iowa', 'KS':'Kansas', 'KY':'Kentucky','LA':'Louisiana', 'ME':'Maine', 'MD':'Maryland', 'MA':'Massachusetts', 'MI':'Michigan', 'MN':'Minnesota', 'MS':'Mississippi', 'MO':'Missouri', 'MT':'Montana', 'NE':'Nebraska', 'NV':'Nevada', 'NH':'New Hampshire', 'NJ':'New Jersey', 'NM':'New Mexico',
			  'NY':'New York', 'NC':'North Carolina', 'ND':'North Dakota', 'OH':'Ohio', 'OK':'Oklahoma', 'OR':'Oregon', 'PA':'Pennsylvania', 'RI':'Rhode Island', 'SC':'South Carolina', 'SD':'South Dakota', 'TN':'Tennessee', 'TX':'Texas', 'UT':'Utah', 'VT':'Vermont', 'VA':'Virginia', 'WA':'Washington', 'WV':'West Virginia', 'WI':'Wisconsin', 'WY':'Wyoming' };
		for (var key in UsaArray) {
		  bd_stateElement.options[bd_stateElement.length] = new Option(UsaArray[key],key);
		}
		if(state != ""){
			jQuery( '#'+bd_stateElementId+' option[value="'+state+'"]').attr('selected', 'selected');
		}else{
			jQuery( '#'+bd_stateElementId+' option[value="CA"]').attr('selected', 'selected');
		}
	}else{
		var bd_state_arr = s_a[selectedbd_countryIndex].split("|");
		for (var i=0; i<bd_state_arr.length; i++) {
			bd_stateElement.options[bd_stateElement.length] = new Option(bd_state_arr[i],bd_state_arr[i]);
		}
		if(state != ""){
			jQuery( '#'+bd_stateElementId+' option[value="'+state+'"]').attr('selected', 'selected');
		}else{
...
function populatecountries(bd_countryElementId, bd_stateElementId, country, state){
	/* given the id of the <select> tag as function argument, it inserts <option> tags */
	var bd_countryElement = document.getElementById(bd_countryElementId);
	bd_countryElement.length=0;
	bd_countryElement.options[0] = new Option('Select Country','');
	bd_countryElement.selectedIndex = 1;
	for (var i=0; i<bd_country_arr.length; i++) {
		bd_countryElement.options[bd_countryElement.length] = new Option(bd_country_arr[i],bd_country_arr[i]);
	}
	if(country != undefined){
		jQuery( '#'+bd_countryElementId+' option[value="'+country+'"]').attr('selected', 'selected');
		populatebd_states( bd_countryElementId, bd_stateElementId, state );
	}else{
		jQuery( '#'+bd_countryElementId+' option[value="USA"]').attr('selected', 'selected');
		populatebd_states( bd_countryElementId, bd_stateElementId, state );
	}
	/* Assigned all countries. Now assign event listener for the bd_states. */
	if( bd_stateElementId ){
		jQuery(bd_countryElement).change(function () {
			populatebd_states( bd_countryElementId, bd_stateElementId, state );
		});
	}
}
