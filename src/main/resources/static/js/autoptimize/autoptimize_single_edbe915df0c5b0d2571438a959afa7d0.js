...
return _moduloSeconds(seconds,settings);}},scrollDefault:null,selectOnBlur:false,show2400:false,showDuration:false,showOn:['click','focus'],showOnFocus:true,step:30,stopScrollPropagation:false,timeFormat:'g:ia',typeaheadHighlight:true,useSelect:false,wrapHours:true};var methods={init:function(options)
...
self.on('cut.timepicker',_keyuphandler);self.on('paste.timepicker',_keyuphandler);_formatValue.call(self.get(0),null,'initial');}});},show:function(e)
...
{list.find('li').removeClass('ui-timepicker-selected');var settings=self.data('timepicker-settings');var timeValue=_time2int(_getTimeValue(self),settings);if(timeValue===null){return;}
...
{if(self.is('input')){self.val(value);var settings=self.data('timepicker-settings');if(settings.useSelect&&source!='select'){self.data('timepicker-list').val(_roundAndFormatTime(_time2int(value),settings));}}
